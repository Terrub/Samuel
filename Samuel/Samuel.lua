...
local _applyParryHaste = function(combat_log_str)
    if stringFind(combat_log_str, "parry") then
...
local _updateDisplay = function()
    ...
    while (_updateRunTime >= _update_display_timer) do
        ...
        _ranged_ratio = mathMin((_current_ranged_time / _total_ranged_time), 1);
...
local setRangedMarkerSize = function(time_in_seconds)
    -- Stop arsin about!
    if time_in_seconds == _db[RANGED_MARKER_SIZE] then return end;
